...
typedef struct {
  ...
  Mat *ws;// weights.
  Mat *bs;// bias.
  Mat *as;// a stands for input matrix, the amount of activations is count + 1.
} NN;

#define NN_INPUT(nn) (nn).as[0]
#define NN_OUTPUT(nn) (nn).as[(nn).count] 

NN nn_alloc(size_t *arch, size_t arch_count);// arch stands for architecture,not archlinux.
